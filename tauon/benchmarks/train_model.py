...
def train_experiment(opt_name, base_lr, max_steps=3000):
  torch.manual_seed(42)
  model = GPTMini(vocab_size=vocab_size, d_model=512, n_layer=6, n_head=8).to(
      device
  )
  ...
  if opt_name == 'tauon':
    ...
    opt_scalar = torch.optim.AdamW(scalar_params, lr=scalar_base_lr)
  elif opt_name == 'Muon':
  ...
  for step in range(1, max_steps + 1):
    ...
    # Обновление LR для основного оптимизатора
    for param_group in optimizer.param_groups:
      param_group['lr'] = base_lr * lr_factor

    # Обновление LR для скалярного оптимизатора
    if opt_scalar:
      for param_group in opt_scalar.param_groups:
        param_group['lr'] = scalar_base_lr * lr_factor
...
# Корректные начальные значения LR для каждого метода
experiments = [('tauon', 0.02), ('Muon', 0.02), ('AdamW', 0.0006)]

results = {}

print('\n' + '=' * 70)
print(
    ' 🔥 BENCHMARK: GPT-MINI (D_MODEL=512) ON TINYSHAKESPEARE'
    f' ({MAX_STEPS} STEPS)'
)
print('=' * 70)
...
for opt_name, lr in experiments:
  ...
  print(
      f"  ✓ Завершено за {res['total_time']:.2f} сек. | Final Val Loss:"
...
ax1.plot(
    steps_tauon,
    loss_tauon,
    label=f"Tauon (LR={exp_dict['tauon']})",
    color=colors['tauon'],
    linewidth=2,
)
ax1.plot(
...
ax1.plot(
    steps_adamw,
    loss_adamw,
    label=f"AdamW (LR={exp_dict['AdamW']})",
    color=colors['AdamW'],
    linewidth=1.5,
    alpha=0.8,
)
...
ax2.plot(
    time_tauon,
    loss_tauon,
    label=f"Tauon ({results['tauon']['total_time']:.0f}s)",
    color=colors['tauon'],
    linewidth=2,
)
ax2.plot(
    time_muon,
    loss_muon,
    label=f"Muon ({results['Muon']['total_time']:.0f}s)",
    color=colors['Muon'],
    linewidth=2,
    linestyle='--',
)
ax2.plot(
    time_adamw,
    loss_adamw,
    label=f"AdamW ({results['AdamW']['total_time']:.0f}s)",
    color=colors['AdamW'],
    linewidth=1.5,
    alpha=0.8,
)
...
plt.suptitle(
    'GPT-Mini (d_model=512, 6 Layers) Benchmark on TinyShakespeare',
    fontsize=14,
    fontweight='bold',
    y=1.03,
)
